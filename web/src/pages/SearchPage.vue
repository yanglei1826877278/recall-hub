...
interface JournalSearchItem { id:number; journalDate:string; content:string; userEdited:boolean; updatedAt:string }
const journalItems=ref<JournalSearchItem[]>([])
async function search(){if(!q.value.trim())return;loading.value=true;try{const data=await unwrap<{items:Entry[];journalItems?:JournalSearchItem[];total:number}>(api.get('/search',{params:{q:q.value,type:type.value||undefined}}));items.value=data.items;journalItems.value=data.journalItems||[];total.value=data.total;searched.value=true}finally{loading.value=false}}
