...
import { Bot, Check, Cloud, Database, Download, KeyRound, LoaderCircle, Moon, Palette, Plus, Save, Send, ShieldCheck, Sparkles, Sun, Trash2, Upload, Wifi, WifiOff } from 'lucide-vue-next'
...
async function testTarget(target:any){
  testingTargetId.value=target.id
  try{
    await unwrap(api.post(`/notification-targets/${target.id}/test`))
    notice.value=`测试消息已发送到“${target.name}”`
  }catch(error:any){
    notice.value=`测试失败：${error.userMessage||'请检查渠道配置'}`
  }finally{
...
.target-test-btn{height:34px;display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:0 12px;border:1px solid color-mix(in oklch,var(--primary) 24%,var(--border));border-radius:calc(var(--radius)*.55);background:color-mix(in oklch,var(--primary) 7%,var(--card));color:var(--primary);font-size:10px;font-weight:850;white-space:nowrap;transition:.18s}.target-test-btn:hover:not(:disabled){border-color:color-mix(in oklch,var(--primary) 48%,var(--border));background:color-mix(in oklch,var(--primary) 12%,var(--card));transform:translateY(-1px)}.target-test-btn:disabled{cursor:wait;opacity:.62}.test-spinner{animation:spin .7s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
