...
<template>
  <div :class="['reader',{night}]">
    ...
    <main class="reader-sheet">
      <div v-if="loading" class="reader-state"><span class="spinner"/>正在翻开这一天…</div>
      <div v-else-if="error" class="reader-state error-state">{{error}}</div>
      <template v-else>
        ...
        <section v-if="content" class="journal-reading" :style="{fontSize:`${fontSize}px`}">
          ...
          <footer class="journal-signoff">
            <span>记于</span><strong>{{date}}</strong><i />
          </footer>
        </section>
